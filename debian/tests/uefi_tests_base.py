...
class UEFITestsBase(unittest.TestCase):
    '''
    Common functionality for linux-uki test cases
    '''
    ...
    def setUpClass(klass):
        ...
        if klass.arch_machine == 'x86_64':
            ...
            raise UEFINotAvailable(feature='any UEFI LinuxUki features', arch=klass.arch_machine)
        ...
        # LinuxUki paths
        klass.linux-uki_pkg_dir = os.path.join('/', 'usr', 'lib', 'linux-uki')
        klass.unsigned_linux-uki_path = os.path.join(klass.linux-uki_pkg_dir, 'linux-uki%s.efi' % klass.arch_suffix)
        klass.signed_linux-uki_path = os.path.join(klass.linux-uki_pkg_dir, 'linux-uki%s.efi.signed' % klass.arch_suffix)
        klass.installed_linux-uki = os.path.join(klass.uefi_install_dir, 'linux-uki%s.efi' % klass.arch_suffix)
        klass.removable_linux-uki = os.path.join(klass.uefi_boot_dir, 'boot%s.efi' % klass.arch_suffix)
