...
class TestSignatures(UEFITestsBase):
    ...
    def testInstalledLinuxUkiIsSigned(self):
        """Check that the installed linux-uki is signed"""
        installed_linux-uki_file = Path(self.signed_linux-uki_path)
        self.assertTrue(installed_linux-uki_file.exists())
        signed_out = subprocess.run(['sbverify', '--list', self.signed_linux-uki_path],
                                    stdout=subprocess.PIPE)
        self.assertIn(b'image signature issuers:', signed_out.stdout)

    def testHaveSignedLinuxUkiOnESP(self):
        """Verify that packaging has provided a signed linux-uki"""
        return
        signed_linux-uki_file = Path(self.installed_linux-uki)
        self.assertTrue(signed_linux-uki_file.exists())

    def testSignaturesExist(self):
        """Validate that a binary has non-zero signatures"""
        unsigned_out = subprocess.run(['sbverify', '--list', self.unsigned_linux-uki_path],
    ...
    def testSignatureIsReplayable(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with open(os.path.join(tmpdirname, 'out.crt'), 'ab+') as certstore:
                ...
                self.assertSignatureOK(os.path.join(tmpdirname, 'out.crt'), self.signed_linux-uki_path)
